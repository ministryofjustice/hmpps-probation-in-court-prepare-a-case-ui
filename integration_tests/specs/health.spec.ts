...
test.describe('Health', () => {
  ...
  test.describe('All healthy', () => {
    test.beforeEach(async () => {
      await Promise.all([hmppsAuth.stubPing(), userPreferencesApi.stubPing(), tokenVerification.stubPing()])
    })
    ...
    test('Info is accessible', async ({ page }) => {
      ...
      const payload = await response.json()
      expect(payload.build.name).toBe('hmpps-probation-in-court-prepare-a-case-ui')
    })
  })

  test.describe('Some unhealthy', () => {
    test.beforeEach(async () => {
      await Promise.all([hmppsAuth.stubPing(), userPreferencesApi.stubPing(), tokenVerification.stubPing(500)])
    })
